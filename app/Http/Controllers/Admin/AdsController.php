...
class AdsController extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->model = new Ads();
        $this->nameItem = 'Quảng cáo';

        View::share('nameClass', 'ads');
    }
    ...
    public function update(AdsRequest $request, string $uuid)
    {
        ...
        if ($request->hasFile('avatar')) {
            if ($request->hasFile('avatar')) {
                $image_path = public_path('images/ads') . '/' . $current->avatar;
                $imageName = time() . '-' . $request->avatar->getClientOriginalName();

                $request->avatar->move(public_path('images/ads'), $imageName);
