...
class CateProductRequest extends FormRequest
{
    ...
    {
        return [
            'name_vn' => request()->route('uuid') ? 'required|unique:tp_category_product,name_vn,' . request()->route('uuid') . ',uuid|max:255' : 'required|unique:tp_category_product|max:255', // Thêm quy tắc max với giá trị là 255
            'slug' => request()->route('uuid') ? 'required|unique:tp_category_product,slug,' . request()->route('uuid') . ',uuid' : 'required|unique:tp_category_product',
